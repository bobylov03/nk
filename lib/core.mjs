...
/* Итог за рацион: цена дня × дни × коэффициент за человек × коэффициент за приёмы */
export function tariffSum(db, tariff, profile) {
  const pr = pricingOf(db);
  const persons = +((profile && profile.persons) || 1);
...
/* Минимальная цена тарифа — та самая «от» на карточке */
export function tariffFrom(db, tariff) {
  const pr = pricingOf(db);
  const kp = Math.min(...Object.values(pr.persons).map(Number).filter(n => n > 0));
...
function normalizeExtra(b, base) {
  const e = { ...base };
  ['name', 'desc', 'img', 'unit', 'badge', 'availableFrom', 'availableTo'].forEach(k => {
...
export async function handle({ method: M, path, body = {}, query = {}, token = '', db, secret, adminPassword, images = null }) {
  ...
  if (M === 'POST' && p === '/api/orders') {
    ...
    const order = {
      ...
      sum: tariffSum(db, t, profile),
      persons: profile.persons, mealsPerDay: profile.meals,
  ...
  if (isClientRoute) {
    ...
    if (M === 'POST' && seg[4] === 'extend') {
      ...
      const order = {
        ...
        tariffId: t.id, planDays: t.days, meals: (c.profile && c.profile.meals) || t.meals, exclude: [],
        sum: tariffSum(db, t, c.profile), persons: (c.profile && c.profile.persons) || 1,

...
export const MAX_MEMBERS = 3;          // всего едоков в одном аккаунте

/* Метки для позиций NC EXTRA. Список открытый — админка позволяет свою. */
export const EXTRA_BADGES = ['новинка', 'сезонное', 'ограниченная партия', 'хит'];
export const RECURRING = ['once', 'weekly', 'every'];   // задел на регулярные добавления
...
const extraOf = (db, id) => (db.extras || []).find(e => e.id === id) || null;

/* Час по Тбилиси — нужен для отсечки в 17:00 предыдущего дня */
const hourNow = () => +new Intl.DateTimeFormat('en-GB', { timeZone: TZ, hour: '2-digit', hour12: false }).format(new Date());

/* Ближайшая доставка, к которой ещё можно что-то добавить.
   По условиям сервиса правки принимаются до 17:00 предыдущего дня. */
export function nextDeliveryDate(db, c) {
  const profile = { ...emptyProfile(), ...(c.profile || {}) };
  const w = profile.weekdays && profile.weekdays.length ? profile.weekdays : [1, 2, 3, 4, 5, 6, 7];
  const start = hourNow() < 17 ? 1 : 2;
  for (let i = start; i <= start + 13; i++) {
    const date = plusDays(i);
    const dow = new Date(date + 'T12:00:00Z').getUTCDay();
    if (w.includes(dow === 0 ? 7 : dow)) return date;
  }
  return plusDays(start);
}

/* Доступна ли позиция в конкретный день */
export function extraAvailable(e, date) {
  if (!e || e.hidden) return false;
  if (e.availableFrom && date < e.availableFrom) return false;
  if (e.availableTo && date > e.availableTo) return false;
  const w = e.availableWeekdays || [];
  if (w.length) {
    const dow = new Date(date + 'T12:00:00Z').getUTCDay();
    if (!w.includes(dow === 0 ? 7 : dow)) return false;
  }
  if (e.stock !== null && e.stock !== undefined && +e.stock <= 0) return false;
  return true;
}

/* Витрина: что показать клиенту. member — есть ли активный рацион (своя цена). */
export function extraCatalog(db, date, member) {
  return (db.extras || [])
    .filter(e => extraAvailable(e, date))
    .sort((a, b) => (a.sort || 0) - (b.sort || 0))
    .map(e => ({
      id: e.id, name: e.name, desc: e.desc, img: e.img, unit: e.unit,
      price: +e.price || 0,
      priceMember: e.priceMember ? +e.priceMember : null,
      yourPrice: (member && e.priceMember) ? +e.priceMember : (+e.price || 0),
      badge: e.badge || '',
      stock: (e.stock === null || e.stock === undefined) ? null : +e.stock,
      limit: +e.perClientLimit || 0
    }));
}
...
/* Коэффициенты цены. Меняются в админке, поэтому в коде их нет. */
export function pricingOf(db) {
  const p = db.pricing || {};
  return {
    persons: { 1: 1, 2: 2, 3: 3, ...(p.persons || {}) },
    meals: { 3: 1, 4: 1, 5: 1, ...(p.meals || {}) }
  };
}

/* Сколько доставок попадёт в календарный период тарифа.
   «Две недели» при графике пн–пт — это 10 доставок, а не 14. */
export const personsOf = (c) => 1 + ((c && c.members) ? c.members.length : 0);

export function tariffDeliveries(tariff, profile) {
  const w = (profile && profile.weekdays && profile.weekdays.length) ? profile.weekdays.length : 7;
  return Math.max(1, Math.round((+tariff.days || 0) * w / 7));
}

/* Итог: цена дня × доставки × коэффициент за человек × коэффициент за приёмы */
export function tariffSum(db, tariff, profile, persons) {
  const pr = pricingOf(db);
  persons = +(persons || (profile && profile.persons) || 1);
  const meals = +((profile && profile.meals) || tariff.meals || 4);
  const kp = +pr.persons[persons] || persons;
  const km = +pr.meals[meals] || 1;
  return Math.round((+tariff.perDay || 0) * tariffDeliveries(tariff, profile) * kp * km);
}

/* «От» на лендинге: полный график, один человек, минимальный коэффициент за приёмы */
export function tariffFrom(db, tariff) {
  const pr = pricingOf(db);
  const kp = +pr.persons[1] || 1;
  const km = Math.min(...Object.values(pr.meals).map(Number).filter(n => n > 0));
  return Math.round((+tariff.perDay || 0) * (+tariff.days || 0) * kp * km);
}
...
/* Дополнительные едоки в аккаунте: у каждого своя анкета,
   доставка и график — общие для семьи. */
function sanitizeMembers(list, base) {
  if (!Array.isArray(list)) return base || [];
  return list.slice(0, MAX_MEMBERS - 1).map((m, i) => {
    const old = (base || [])[i] || {};
    return {
      id: old.id || uid('m'),
      name: str(m.name).slice(0, 80) || old.name || ('Едок ' + (i + 2)),
      profile: sanitizeProfile(m.profile, old.profile)
    };
  });
}
  ...
  if (b.kbju) {
    ...
    p.kbju = {
      ...
      calc: {
        ...
        at: (cal.sex && cal.age && cal.height && cal.weight)
          ? (str(cal.at) || new Date().toISOString())
          : str(cal.at)
      }
    };
  }
...
function normalizeExtra(b, base) {
  const e = { ...base };
  ['name', 'desc', 'img', 'unit', 'badge', 'availableFrom', 'availableTo',
   'name_en', 'name_ka', 'desc_en', 'desc_ka', 'unit_en', 'unit_ka'].forEach(k => {
    if (k in b) e[k] = str(b[k]).slice(0, 400);
  });
  if ('price' in b) e.price = Math.max(0, +b.price || 0);
  if ('priceMember' in b) e.priceMember = Math.max(0, +b.priceMember || 0);
  if ('perClientLimit' in b) e.perClientLimit = Math.max(0, +b.perClientLimit || 0);
  if ('sort' in b) e.sort = +b.sort || 0;
  if ('hidden' in b) e.hidden = !!b.hidden;
  if ('stock' in b) {
    e.stock = (b.stock === null || b.stock === '' || b.stock === undefined) ? null : Math.max(0, +b.stock || 0);
  }
  if ('availableWeekdays' in b) {
    const w = (Array.isArray(b.availableWeekdays) ? b.availableWeekdays : []).map(Number).filter(n => n >= 1 && n <= 7);
    e.availableWeekdays = [...new Set(w)].sort((a, x) => a - x);
  }
  return e;
}
...
function dossier(db, c) {
  ...
  return {
    ...
    members: (c.members || []).map(m => ({
      ...
      dietLabel: dietLabel((m.profile || {}).diet)
    })),
    persons: personsOf(c),
...
/* =======================================================================
   ...
   ======================================================================= */
export async function handle({ method: M, path, body = {}, query = {}, token = '', db, secret, adminPassword, images = null }) {
  ...
  if (M === 'GET' && p === '/api/public') {
    return ok({
      ...
      pricing: pricingOf(db),
      tariffsFrom: db.tariffs.map(t => ({ id: t.id, from: tariffFrom(db, t) }))
    });
  }
  if (M === 'GET' && p === '/api/dishes') return ok({ dishes: db.dishes, cats: CAT_RU });

  /* NC EXTRA — витрина. Без рациона показываем обычную цену. */
  if (M === 'GET' && p === '/api/extras') {
    return ok({ extras: extraCatalog(db, today(), false), badges: EXTRA_BADGES });
  }

  /* Загруженные фото. Отдаются всем, загружает только админ. */
  if (M === 'GET' && /^\/api\/images\/[^/]+$/.test(p)) {
    if (!images) return err(404, 'Хранилище изображений недоступно');
    const file = await images.get(seg[3]);
    return file
      ? { code: 200, data: null, changed: false, binary: file }
      : err(404, 'Изображение не найдено');
  }
  ...
  if (M === 'GET' && p === '/api/health') {
    return ok({
      ...
      usingSeedPassword: String(adminPassword || '').trim() === 'CHANGE_ME_IN_NETLIFY_ENV',
  ...
  if (M === 'POST' && p === '/api/orders') {
    ...
    const persons = 1 + members.length;
    ...
    const order = {
      ...
      tariffId: t.id,
      planDays: tariffDeliveries(t, profile),      // столько доставок оплачено
      calendarDays: t.days,                        // за какой период они распределены
      weekdays: profile.weekdays,
      meals: profile.meals || +body.meals || t.meals,
      exclude: profile.dislikes.length ? profile.dislikes : arr(body.exclude),
      promo: str(body.promo).toUpperCase(),
      sum: tariffSum(db, t, profile, persons),
      persons, members, mealsPerDay: profile.meals,
  ...
  if (M === 'POST' && /^\/api\/orders\/[^/]+\/pay$/.test(p)) {
    ...
    if (!c) {
      c = {
        ...
        favourites: [], members: o.members || [], cycleStart: today(), cycleDone: false
      };
      ...
      if (o.members) c.members = o.members;
      c.cycleStart = today(); c.cycleDone = false;
      logChange(c, 'Продление на ' + o.planDays + ' дней');
    }
    c.history = c.history || [];
    c.history.unshift({
      title: 'NC · ' + o.planDays + ' ' + plural(o.planDays, 'доставка', 'доставки', 'доставок'),
      meta: dateRu(today()) + ' · ' + o.meals + ' приёма'
        + (o.persons > 1 ? ' · ' + o.persons + ' чел.' : ''), sum: o.sum
    });
    o.clientId = c.id;
  ...
  if (isClientRoute) {
    ...
    if (M === 'PUT' && seg[4] === 'members') {
      c.members = sanitizeMembers(body.members, c.members);
      logChange(c, 'Состав едоков: ' + personsOf(c) + ' чел.');
      return saved({ members: c.members, persons: personsOf(c) });
    }
    ...
    /* NC EXTRA в кабинете: витрина с ценой участника и уже добавленное */
    if (M === 'GET' && seg[4] === 'extras') {
      const date = nextDeliveryDate(db, c);
      const member = (c.daysLeft || 0) > 0 && !c.paused;
      const mine = (db.extraOrders || []).filter(o => o.clientId === c.id && o.status !== 'cancelled');
      return ok({
        deliveryDate: date,
        deliveryLabel: dateRu(date, { weekday: 'long', day: 'numeric', month: 'long' }),
        member,
        extras: extraCatalog(db, date, member),
        orders: mine.filter(o => o.deliveryDate >= today())
      });
    }
    if (M === 'POST' && seg[4] === 'extras') {
      const date = str(body.deliveryDate) || nextDeliveryDate(db, c);
      const member = (c.daysLeft || 0) > 0 && !c.paused;
      const recurring = RECURRING.includes(body.recurring) ? body.recurring : 'once';
      const raw = Array.isArray(body.items) ? body.items : [];
      if (!raw.length) return err(400, 'Ничего не выбрано');

      const items = [];
      for (const it of raw) {
        const e = extraOf(db, it.extraId);
        if (!e) return err(404, 'Позиция не найдена');
        if (!extraAvailable(e, date)) return err(409, '«' + e.name + '» недоступна на ' + dateRu(date));
        const qty = Math.max(1, Math.min(99, +it.qty || 1));
        if (e.stock !== null && e.stock !== undefined && qty > +e.stock) {
          return err(409, '«' + e.name + '»: осталось ' + e.stock + ' шт.');
        }
        if (+e.perClientLimit) {
          const already = (db.extraOrders || [])
            .filter(o => o.clientId === c.id && o.status !== 'cancelled' && o.deliveryDate === date)
            .flatMap(o => o.items).filter(x => x.extraId === e.id)
            .reduce((a, x) => a + x.qty, 0);
          if (already + qty > +e.perClientLimit) {
            return err(409, '«' + e.name + '»: не более ' + e.perClientLimit + ' шт. на клиента');
          }
        }
        const price = (member && e.priceMember) ? +e.priceMember : (+e.price || 0);
        items.push({ extraId: e.id, name: e.name, unit: e.unit || '', qty, price, sum: price * qty });
      }

      const order = {
        id: uid('x'), clientId: c.id, deliveryDate: date, status: 'pending',
        items, sum: items.reduce((a, x) => a + x.sum, 0),
        recurring, standalone: false,          // задел: заказы без рациона
        createdAt: new Date().toISOString()
      };
      (db.extraOrders = db.extraOrders || []).unshift(order);
      logChange(c, 'NC EXTRA к доставке ' + dateRu(date) + ': ' + items.map(i => i.name + '×' + i.qty).join(', '));
      return saved({ order });
    }
    if (M === 'POST' && seg[4] === 'extras-pay') {
      const o = (db.extraOrders || []).find(x => x.id === str(body.orderId) && x.clientId === c.id);
      if (!o) return err(404, 'Заказ не найден');
      if (o.status === 'paid') return err(400, 'Уже оплачен');
      o.status = 'paid'; o.pay = str(body.pay) || 'card'; o.paidAt = new Date().toISOString();
      o.items.forEach(it => {
        const e = extraOf(db, it.extraId);
        if (e && e.stock !== null && e.stock !== undefined) e.stock = Math.max(0, +e.stock - it.qty);
      });
      return saved({ order: o });
    }
    if (M === 'POST' && seg[4] === 'extras-cancel') {
      const o = (db.extraOrders || []).find(x => x.id === str(body.orderId) && x.clientId === c.id);
      if (!o) return err(404, 'Заказ не найден');
      if (o.status === 'paid') return err(400, 'Оплаченный заказ отменяет шеф');
      o.status = 'cancelled';
    ...
    if (M === 'POST' && seg[4] === 'extend') {
      ...
      const order = {
        ...
        tariffId: t.id,
        planDays: tariffDeliveries(t, c.profile), calendarDays: t.days,
        weekdays: (c.profile && c.profile.weekdays) || [1, 2, 3, 4, 5, 6, 7],
        meals: (c.profile && c.profile.meals) || t.meals, exclude: [],
        sum: tariffSum(db, t, c.profile, personsOf(c)),
        persons: personsOf(c), members: c.members || [],
        note: 'Продление рациона', profile: c.profile || emptyProfile()
      };
      db.orders.unshift(order);
      return saved({ order });
    }
  ...
  if (M === 'POST' && p === '/api/admin/login') {
    // обе стороны обрезаем: в переменную окружения легко попадает пробел или \n
    if (str(body.password) !== String(adminPassword || '').trim()) return err(401, 'Неверный пароль');
  ...
  if (M === 'GET' && p === '/api/admin/state') {
    ...
    return ok({
      ...
      promos: db.promos, options: OPTS, pricing: pricingOf(db),
      extras: db.extras || [], extraBadges: EXTRA_BADGES,
      extraOrders: (db.extraOrders || []).map(o => {
        const cl = db.clients.find(x => x.id === o.clientId);
        return { ...o, clientName: cl ? cl.name : '—' };
      }),
  ...
  /* NC EXTRA — управление */
  if (M === 'POST' && p === '/api/extras') {
    const e = normalizeExtra(body, {
      id: uid('e'), name: '', desc: '', img: '', unit: '', price: 0, priceMember: 0,
      stock: null, perClientLimit: 0, availableFrom: '', availableTo: '',
      availableWeekdays: [], hidden: false, badge: '', sort: (db.extras || []).length
    });
    (db.extras = db.extras || []).push(e);
    return saved({ extra: e });
  }
  if (M === 'PUT' && /^\/api\/extras\/[^/]+$/.test(p)) {
    const e = extraOf(db, seg[3]);
    if (!e) return err(404, 'Позиция не найдена');
    Object.assign(e, normalizeExtra(body, e));
    return saved({ extra: e });
  }
  if (M === 'DELETE' && /^\/api\/extras\/[^/]+$/.test(p)) {
    db.extras = (db.extras || []).filter(x => x.id !== seg[3]);
    return saved({ ok: true });
  }
  if (M === 'GET' && p === '/api/extra-orders') {
    const list = (db.extraOrders || []).map(o => {
      const c = db.clients.find(x => x.id === o.clientId);
      return { ...o, clientName: c ? c.name : '—', clientPhone: c ? c.phone : '' };
    });
    return ok({ orders: list });
  }
  if (M === 'POST' && /^\/api\/extra-orders\/[^/]+\/(paid|cancel)$/.test(p)) {
    const o = (db.extraOrders || []).find(x => x.id === seg[3]);
    if (!o) return err(404, 'Заказ не найден');
    o.status = p.endsWith('paid') ? 'paid' : 'cancelled';
  ...
  if (M === 'POST' && p === '/api/images') {
    if (!images) return err(503, 'Хранилище изображений недоступно');
    const m = String(body.data || '').match(/^data:(image\/(png|jpeg|jpg|webp));base64,(.+)$/);
    if (!m) return err(400, 'Ожидается изображение в формате JPEG, PNG или WebP');
    const buf = Buffer.from(m[3], 'base64');
    if (buf.length > 4 * 1024 * 1024) return err(413, 'Файл больше 4 МБ — уменьшите изображение');
    const ext = m[2] === 'jpeg' ? 'jpg' : m[2];
    const key = 'up_' + uid('') + '.' + ext;
    await images.put(key, buf, m[1]);
    return ok({ img: key, size: buf.length });
  }
  if (M === 'DELETE' && /^\/api\/images\/[^/]+$/.test(p)) {
    if (!images) return err(503, 'Хранилище изображений недоступно');
    const key = seg[3];
    if (db.dishes.some(d => d.img === key)) return err(409, 'Фото используется блюдом — сначала замените его');
    await images.del(key);
    return ok({ ok: true });
  }
  if (M === 'POST' && p === '/api/dishes') {
    ...
    // переводы при создании — иначе их пришлось бы дописывать вторым заходом
    ['name_en', 'name_ka', 'desc_en', 'desc_ka', 'ing_en', 'ing_ka', 'why_en', 'why_ka']
      .forEach(k => { if (k in body) d[k] = str(body[k]); });
    db.dishes.unshift(d);
    return saved({ dish: d });
  }
  ...
  if (M === 'PUT' && /^\/api\/dishes\/[^/]+$/.test(p)) {
    const d = dishOf(db, seg[3]);
    if (!d) return err(404, 'Блюдо не найдено');
    ['name', 'cat', 'ing', 'img', 'desc', 'why', 'cuisine',
     'name_en', 'name_ka', 'desc_en', 'desc_ka', 'ing_en', 'ing_ka', 'why_en', 'why_ka']
      .forEach(k => { if (k in body) d[k] = str(body[k]); });
  ...
  if (M === 'PUT' && p === '/api/pricing') {
    const norm = (obj, keys) => {
      const out = {};
      keys.forEach(k => {
        const v = +(obj || {})[k];
        out[k] = (isFinite(v) && v > 0) ? Math.round(v * 100) / 100 : 1;
      });
      return out;
    };
    db.pricing = {
      persons: norm(body.persons, ['1', '2', '3']),
      meals: norm(body.meals, ['3', '4', '5'])
    };
    return saved({ pricing: db.pricing });
  }

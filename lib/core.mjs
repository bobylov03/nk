...
export function clientDashboard(db, id) {
  ...
  return {
    ...
    collection: mine.filter(r => r.stars === 5 || r.mark === 'love')
...
function sanitizeProfile(input, base) {
  ...
  ['medicalNote', 'dislikesNote', 'likesNote', 'never', 'favDishes', 'extra', 'weight'].forEach(k => {
  ...
  if (b.kbju) {
    p.kbju = {
      on: !!b.kbju.on,
      kcal: +b.kbju.kcal || 0, p: +b.kbju.p || 0, f: +b.kbju.f || 0, c: +b.kbju.c || 0,
      note: str(b.kbju.note).slice(0, 2000)
    };
  }
  if (b.delivery) {
    const d = b.delivery;
...
export async function handle({ method: M, path, body = {}, query = {}, token = '', db, secret, adminPassword }) {
  ...
  if (M === 'POST' && p === '/api/orders') {
    ...
    if (order.promo && db.promos[order.promo]) {
      order.sum = Math.round(order.sum * (1 - db.promos[order.promo] / 100));
    }
